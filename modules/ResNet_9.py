...
class RCABP(nn.Module):
	def __init__(self,inf,onf):
		...
		self.proc1 = nn.Sequential(nn.Conv2d(inf,onf,(3,3),padding=1,stride=1), nn.GroupNorm(4,onf),nn.PReLU(),\
			nn.Conv2d(onf,onf,(3,3),padding=1,stride=1), nn.GroupNorm(4,onf), nn.PReLU(),\
			nn.Conv2d(onf,onf,(3,3),padding=1,stride=1), nn.GroupNorm(4,onf), nn.PReLU());
		self.proc2 = nn.Sequential(nn.Conv2d(inf,onf,(3,3),padding=1,stride=1), nn.GroupNorm(4,onf), nn.PReLU(),\
			nn.Conv2d(onf,onf,(3,3),padding=1,stride=1), nn.GroupNorm(4,onf), nn.PReLU(),\
			nn.Conv2d(onf,onf,(3,3),padding=1,stride=1), nn.GroupNorm(4,onf), nn.PReLU());
...
class RCA(nn.Module):
	def __init__(self,nf,act='lrelu'):
		super(RCA,self).__init__();
		self.acts = nn.ModuleDict({'lrelu':nn.LeakyReLU(negative_slope=0.2), 'prelu':nn.PReLU()});
		self.proc = nn.Sequential(nn.Conv2d(nf,nf,(3,3),padding=1,stride=1), nn.GroupNorm(4,nf), self.acts[act],\
			nn.Conv2d(nf,nf,(3,3),padding=1,stride=1), nn.GroupNorm(4,nf), self.acts[act],\
			nn.Conv2d(nf,nf,(3,3),padding=1,stride=1), nn.GroupNorm(4,nf), self.acts[act]\
			);
		self.avg = nn.AdaptiveAvgPool2d((1,1));
		self.bottleneck = nn.Sequential(nn.Linear(nf,nf//4),self.acts[act]);
...
class Enc(nn.Module):
	def __init__(self,nf):
		...
		self.d1 = nn.Sequential(\
			ResBlock(nf),\
			nn.Conv2d(nf,nf,(3,3),padding=1,stride=2), nn.GroupNorm(4,nf), nn.PReLU()\
			);
		...
		self.u1 = nn.Sequential(\
			...
			nn.Conv2d(nf,nf,(3,3),padding=1,stride=1), nn.GroupNorm(4,nf), nn.PReLU()\
			);
	...
	def forward(self,input_tensor):
		...
		input_tensor = temp4 + 0.2*input_tensor;
...
class Dec(nn.Module):
	def __init__(self,nf):
		...
		self.begin = nn.Sequential(nn.Conv2d(nf,nf,(1,1)), nn.PReLU());

		self.l1 = DenseBlock(nf);

		self.up_1 = nn.Sequential(nn.Upsample(scale_factor=2, mode='nearest'), nn.Conv2d(nf,nf,(5,5),padding=2,stride=1), nn.GroupNorm(4,nf), nn.PReLU());
		self.up_2 = nn.Sequential(nn.Upsample(scale_factor=2, mode='nearest'), nn.Conv2d(nf,nf,(5,5),padding=2,stride=1), nn.GroupNorm(4,nf), nn.PReLU());
