...
class Dec(nn.Module):
	def __init__(self,nf):
		...
		self.up_1 = nn.Sequential(nn.Upsample(scale_factor=2, mode='nearest'), nn.Conv2d(nf,nf,(5,5),padding=2,stride=1), nn.BatchNorm2d(nf), nn.PReLU());
		self.l2 = DenseBlock(nf);
		self.up_2 = nn.Sequential(nn.Upsample(scale_factor=2, mode='nearest'), nn.Conv2d(nf,nf,(5,5),padding=2,stride=1), nn.BatchNorm2d(nf), nn.PReLU());
