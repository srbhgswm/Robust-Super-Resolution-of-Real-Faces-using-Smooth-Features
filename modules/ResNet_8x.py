...
class Dec(nn.Module):
	def __init__(self,nf):
		...
		self.begin_up = nn.Sequential(nn.Upsample(scale_factor=2, mode='nearest'), nn.Conv2d(nf,nf,(5,5),padding=2,stride=1), nn.BatchNorm2d(nf), nn.PReLU());
	...
	def forward(self,input_tensor):
		input_tensor = self.begin_up(input_tensor);
		input_tensor = self.l1(input_tensor);
